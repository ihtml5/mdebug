        ...
        return (<div className={styles.mdebugActionApplicationCon} style={{
            ...
            <div className={styles.mdebugFixedHeader}>
                <div className={styles.settings}>
                    <input type="text" placeholder="filter application" onChange={this.onChange.bind(this)} className={styles.applicationinput} value={applicationWords} />
                ...
                <ul className={styles.mdebugAction}>
                    {STORAGEMAPPING.map(tabName => <li className={this.getSubSelected(tabName)} onClick={() => this.onSelectTabHandler(tabName)}>{tabName}</li>)}
                </ul>
            </div>
            <div>
                ...
                <table
                    ...
                    {storage.filter(cstorage => {
                        ...
                        return !applicationWords || key.toLowerCase().indexOf(applicationWords.toLowerCase()) !== -1 || value.toLowerCase().indexOf(applicationWords.toLowerCase()) !== -1;
                    }).map(storageUnit => {
                        ...
                        return ( <tr key={storageUnit.key} id={storageUnit.key} className={`${styles.mdebugApplicationTr} ${curKey === storageUnit.key && styles.mdebugTrActive}`} onClick={(e) => this.cellClickHandler(e)}>
                            ...
                            <td className={styles.mdebugApplicationValueName} style={{
                                boder: '1px solid  #eee',
                            }}>
                                {
                                    key === curKey && canEdit ? (
