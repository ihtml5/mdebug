...
const proxyConsole = () => {
  Object.keys(CONSOLEMAPPING).filter(cmapKey => cmapKey !== 'all').forEach((method => {
    console[method] = (...rest) => {
      let emitProxyConsole = rest.length > 0;
      rest.forEach(val => {
        const isReactSelfConsole = isString(val) && (val.indexOf('Check the render method') !== 0 || val.indexOf('Reconciliation') !== 0 || val.indexOf('Cannot update during an existing state transition') !== 0);
    ...
    e => {
      ...
      if (target !== window && target.nodeName && ERRORTYPES[target.nodeName.toUpperCase()]) {
        ...
        emit('network', {
          ...
          httpcode: 'error',
          starttime: 0,
          type: ['link', 'script', 'css', 'img'].indexOf(lowerNodeName) < 0 ? 'media' : lowerNodeName,
